...
def pubeval(race, pos):
    """
    Return pubeval score from computers point of view, meaning
    the player that moves in the negative direction and starts
    at [24].
...
if __name__ == '__main__':
    ...
    g = Game.new()
    player = 'x'
    if player == 'o': # reverse on player_num being true...
        g.draw()
        g.reverse()
        print
    g.draw()
    for i, ent in enumerate(g.grid):
        print 'grid: ', i, ent
    pos = game_to_pos(g)
    print '24: len=', len(g.grid[23]), ' sq 24, should be negative if starting pos', pos[24]
    print pubeval(False, pos)

    moves = g.get_actions((1, 2), 'x', nodups=True)
    for a in sorted([str(foo) for foo in moves]):
        print a
        
    for action in sorted(moves):
        ateList = g.take_action(action, player)
        print 'Action: ', action, ateList
        g.draw()
        g.undo_action(a, player, ateList)
